// unicode_tables.h — GENERATED by oracle-venv python unicodedata. Do not hand-edit.
// Code-point ranges for the Unicode general categories the Laguna pre-tokenizer regex
// needs: \p{L} (any letter) and \p{N} (any number). Generated rather than hand-rolled
// because a wrong range silently changes tokenization on non-ASCII input.
#pragma once
#include <cstdint>
#include <algorithm>
namespace lgtok {
// 659 letter ranges, 137 number ranges (Unicode 15.0.0)
...
static const int kNumberN = 137;

inline bool in_ranges(uint32_t cp, const Range* r, int n) {
    int lo = 0, hi = n - 1;
    while (lo <= hi) { int m = (lo + hi) >> 1;
        if (cp < r[m].lo) hi = m - 1; else if (cp > r[m].hi) lo = m + 1; else return true; }
    return false;
}
inline bool is_letter(uint32_t cp) { return in_ranges(cp, kLetter, kLetterN); }
inline bool is_number(uint32_t cp) { return in_ranges(cp, kNumber, kNumberN); }
inline bool is_space(uint32_t cp) {   // \s as the Rust regex crate defines it (Unicode White_Space)
